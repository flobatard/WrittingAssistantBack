...
| Stockage objet | MinIO (S3-compatible) via boto3 |
...
│   │   └── s3.py                       # Client S3 (boto3) + helpers presigned URLs
│   ├── models/
...
│   ├── env.py
│   └── versions/
├── docker-compose.yml                  # PostgreSQL + ChromaDB + LanguageTool + MinIO + API
├── requirements.txt
├── .env.example
└── Dockerfile
```
...
```bash
cp .env.example .env
docker-compose up -d db chromadb languagetool minio   # Démarrer PostgreSQL + ChromaDB + LanguageTool + MinIO
...
| `S3_ENDPOINT_URL` | `http://localhost:9000` | URL de l'API S3 (MinIO) |
| `S3_PUBLIC_URL` | `http://localhost:9000` | URL publique S3 (utilisée dans les presigned URLs côté browser) |
| `S3_ACCESS_KEY` | `minioadmin` | Clé d'accès S3 |
| `S3_SECRET_KEY` | `minioadmin` | Clé secrète S3 |
| `S3_BUCKET_NAME` | `writing-assistant` | Nom du bucket |
| `S3_PRESIGNED_EXPIRY` | `3600` | Durée de validité des presigned URLs (secondes) |
...
| `attributes` | JSONB nullable | Champs spécifiques au type (ex. `{"age": 35, "role": "antagonist"}`). Peut contenir des clés S3 (`object_key`) référençant des fichiers uploadés. |
...
| `DELETE` | `/books/{book_id}/assets/{asset_id}` | Supprimer (supprime aussi le dossier S3 associé) |
| `POST` | `/books/{book_id}/assets/{asset_id}/upload-url` | Générer une URL presignée d'upload |
| `POST` | `/books/{book_id}/assets/{asset_id}/download-url` | Générer une URL presignée de téléchargement |

**Upload de fichiers :** Chaque asset dispose d'un dossier S3 dédié (`books/{book_id}/assets/{asset_id}/`). Un asset peut avoir plusieurs fichiers. Le front stocke les `object_key` retournées dans `attributes` selon ses besoins. Le client uploade directement vers MinIO via les presigned URLs (PUT). À la suppression d'un asset, tout le dossier S3 est nettoyé. Types autorisés : `image/png`, `image/jpeg`, `image/webp`, `image/gif`, `image/svg+xml`, `application/pdf`.
